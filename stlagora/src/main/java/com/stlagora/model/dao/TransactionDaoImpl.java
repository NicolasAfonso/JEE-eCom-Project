...
public class TransactionDaoImpl extends GenericDaoImpl<Transaction> implements TransactionDao{
	...
	public List<Transaction> findBySeller(User seller) {
		Query q = em.createQuery("SELECT t FROM Transactions t WHERE t.seller_id=:seller_id",Transaction.class);
	...
	public List<Transaction> findByBuyer(User buyer) {
		Query q = em.createQuery("SELECT t FROM Transactions t WHERE t.buyer=:buyer_id",Transaction.class);
	...
	public List<Transaction> findByAmount(float amount) {
		Query q = em.createQuery("SELECT t FROM Transactions t WHERE t.amount=:amount",Transaction.class);
	...
	public List<Transaction> findByDate(Date date) {
		Query q = em.createQuery("SELECT t FROM Transactions t WHERE t.date=:date",Transaction.class);
	...
	public List<Transaction> findByProduct(Product product) {
		Query q = em.createQuery("SELECT t FROM Transactions t WHERE t.product_id=:product",Transaction.class);
